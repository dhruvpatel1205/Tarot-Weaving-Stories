...
def get_card(deck):
    num = random.randint(0, len(deck) - 1)  # -1 will help to choose an unique card
    card = deck[num]
    del (deck[num])  # so we don't get the same card twice if we're calling this multiple times for the same hand
    rev = random.randint(-1, 1)  # card reversed 0-false or 1-true
    drawn = (card, rev)  # tuple of card dictionary + 1 or 0 for reversal
    return drawn


# def get_name():
#   name = [{"name": "The Fool"}, {"name": "The Magician"}, {"name": "The Popess"}]
...
def get_deck():
    deck = [
        ...
        {"name": "Death", "url": "13", "image": "images/13.jpeg",
